...
class PoseSensor(Sensor):
    ...
    def get_observation(
        ...
        # ✅ 安全转换：确保所有值都是标量 float
        try:
            x = float(agent_position_xyz[0])
            z = float(agent_position_xyz[2])
            heading = float(agent_heading)  # ✅ 关键：强制转为标量
            time_step = float(ep_time)
        except Exception as e:
            print(f"[ERROR] Failed to convert observation: {e}")
            print(f"agent_position_xyz: {agent_position_xyz}, type: {type(agent_position_xyz)}")
            print(f"agent_heading: {agent_heading}, type: {type(agent_heading)}")
            # 返回安全默认值
            return np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32)

        # ✅ 构造最终观测
        obs = np.array([-z, x, heading, time_step], dtype=np.float32)

        # ✅ 可选：断言形状正确
        assert obs.shape == (4,), f"obs must be (4,), got {obs.shape}"
