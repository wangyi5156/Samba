...
#### We propose Samba, a hybrid architecture built on state-space models, to overcome the long-standing efficiency and modeling bottlenecks in audio-visual navigation. Samba integrates an adaptive selective Mamba State Encoder with a bidirectional Audio Mamba Encoder; this combination successfully captures global time-frequency correlations where convolutions fail and prevents the representation dilution typical of recurrent units. Our empirical validation shows that updating the backbone not only yields stronger embodied representations but also slashes parameter size and computation. This shift provides an efficient, high-performance reference design for future embodied AI deployment.
